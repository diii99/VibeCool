...
(function() {
  ...
  // Main code
  var vis,
      maxSize = 512, // Original size
      pcDisplaySize = 486, // Size for PC display (95% of original)
      minSize = 4,
      dim = maxSize / minSize;
  ...
  koala.loadImage = function(imageData) {
    ...
    if (sourceWidth > sourceHeight) {
      // Landscape image
      sourceX = Math.floor((sourceWidth - sourceHeight) / 2);
      sourceWidth = sourceHeight;
    } else if (sourceHeight > sourceWidth) {
      // Portrait image
      sourceY = Math.floor((sourceHeight - sourceWidth) / 2);
      sourceHeight = sourceWidth;
    }
    
    // Draw the center-cropped image into a square canvas
    ctx.drawImage(
      imageData,
  ...
  koala.makeCircles = function(selector, colorData, onEvent) {
    ...
    if (!vis) {
      ...
      vis = d3.select(selector)
        .append("svg")
          .attr("width", pcDisplaySize)
          .attr("height", pcDisplaySize)
